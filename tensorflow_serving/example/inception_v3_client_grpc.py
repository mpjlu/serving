...
def main(_):
  ...
  request.inputs['images'].CopyFrom(
      tf.contrib.util.make_tensor_proto(image.astype(dtype=np.float), shape=[1, height, width, 3]))
      #tf.contrib.util.make_tensor_proto(image.astype(dtype=np.float32), shape=[1, height, width, 3]))
  request.inputs['height'].CopyFrom(
      tf.contrib.util.make_tensor_proto(height, shape=[1]))
  request.inputs['width'].CopyFrom(
      tf.contrib.util.make_tensor_proto(width, shape=[1]))
  start = time.time()
  print("send request")
  result = stub.Predict(request, 10)  # 10 secs timeout
  dur1 = time.time() - start
  print("Get Result time: %.6f" % dur1)
  print(result.outputs['segmentation_map'])
